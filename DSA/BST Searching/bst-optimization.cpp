...
int log2(int arraySize) {
	int height = 0;
	while ((1 << height) - 1 < arraySize) {
		++height;
	}
	return height - 1;
}

void buildBinaryTreeArrayRec(const int* hayStack, int count, int* binaryTreeArr, int height, int level = 0, int idx = 0) {
	int middle = count / 2;
	binaryTreeArr[idx] = hayStack[middle];

	if (level + 1 < height) {
		int childrenIdx = (idx * 2) + 1;

		buildBinaryTreeArrayRec(hayStack, middle, binaryTreeArr, height, level + 1, childrenIdx);
		buildBinaryTreeArrayRec(hayStack + middle + 1, count - middle - 1, binaryTreeArr, height, level + 1, childrenIdx + 1);
	}
}

int findMaxBinaryTreeElements(int freeBytes, size_t intSize, size_t taskSize) {
	int low = 1;
	int high = freeBytes / intSize;
	int result = 0;

	while (low <= high) {
		int middle = (low + high) / 2;
		int height = log2(middle) + 1;
		size_t used = middle * intSize + height * taskSize;
...
void buildBinaryTreeArrayBottomUp(const AlignedIntArray &hayStack, int* binaryTreeArr, int btArrSize, int maxHeight, StackAllocator& allocator) {
	const int count = static_cast<int>(hayStack.getCount());

	BuildTask* taskStack = allocator.alloc<BuildTask>(maxHeight); 
	int stackTop = 0;

	taskStack[stackTop] = {0, count, 0};
	++stackTop;

	while (stackTop > 0) {
		BuildTask task = taskStack[--stackTop];

		if (task.count <= 0 || task.index >= btArrSize) {
			continue;
		}

		int middle = task.count / 2;
		binaryTreeArr[task.index] = hayStack[task.start + middle];

		int childrenIdx = (task.index * 2) + 1;

		if (stackTop < maxHeight) {
			taskStack[stackTop] = {task.start + middle + 1, task.count - middle - 1, childrenIdx + 1};
			++stackTop;
		}
		if (stackTop < maxHeight) {
			taskStack[stackTop] = {task.start, middle, childrenIdx};
			++stackTop;
		}
	}
}
...
static void betterSearch(const AlignedIntArray &hayStack, const AlignedIntArray & needles, AlignedIntArray &indices, StackAllocator &allocator) {
	const int hayStackSize = hayStack.getCount();
	int N = log2(hayStackSize);
	int maxBtArrSize = findMaxBinaryTreeElements(allocator.freeBytes(), sizeof(int), sizeof(BuildTask));

	int* binaryTreeArr = allocator.alloc<int>(maxBtArrSize);
	int height = log2(maxBtArrSize);

	if (N < 20) { // recursion is often faster, due to low tree height
		buildBinaryTreeArrayRec(hayStack.get(), hayStackSize, binaryTreeArr, height);
	}
	else { // bottom up can be faster for larger trees
		buildBinaryTreeArrayBottomUp(hayStack, binaryTreeArr, maxBtArrSize, height + 1, allocator);
	}

	for (int idx = 0; idx < needles.getCount(); ++idx) {
		int left = 0, level = 0, btaIdx = 0;

		int number = needles[idx];

		int count = hayStack.getCount();

		while (count > 0) {
			int middle = count / 2;
			int newMiddle = (level < height) ? binaryTreeArr[btaIdx] : hayStack[left + middle];
			++level;
		...
		indices[idx] = (left < hayStackSize && hayStack[left] == number) ? left : -1;
	}
