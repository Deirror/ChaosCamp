...
void task3() {
	...
	gen::createPyramid7(1.2f, 2, triangles);
	...
	crt::PPMFile ppmFile("task3_image.ppm", resolution);

	// camera.dolly(2);
	// camera.truck(2);
	// camera.pedestal(-2);
	// camera.pan(crt::math::to_radians(-45));
	camera.tilt(crt::math::to_radians(45));
	// camera.roll(crt::math::to_radians(45));
	...
	for (int y = 0; y < camera.resolution.height(); ++y) {
		for (int x = 0; x < camera.resolution.width(); ++x) {
			...
			if (closestT != FLT_MAX) {
				int idx = hitRecord.triangle - &triangles[0];
